...
# Check out head of titanic using a pipe!
titanic %>% head()
...
titanic %>% drop_na() %>% summary()

# Wrangling your data ---------------------------

# Subset to get all 'Male' rows
titanic %>% filter(Sex == "male") %>% head()

# Subset to get all 'Female' rows
titanic %>% filter(Sex == "female") %>% head()

# Arrange by increasing Fare
titanic %>% arrange(Fare) %>% head()

# # Arrange by decreasing Fare
titanic %>% arrange(desc(Fare)) %>% head()

# Create new column fam_size (size of family)
titanic %>% mutate(fam_size = Parch + SibSp)

# Create new column fam_size (size of family)
# Arrange by decreasing FamSize
titanic %>% mutate(fam_size = Parch + SibSp) %>%
  arrange(desc(fam_size)) %>%  head()
...
ggplot(titanic1, aes(Sex)) + geom_bar()

# Plot barplot of passenger Sex & stack according to Survival
ggplot(titanic1, aes(Sex)) + geom_bar(aes(fill = Survived))
...
ggplot(titanic1, aes(x = Age, y = Fare, color = Sex)) + geom_point() +
  facet_wrap(~ Survived)
...
titanic %>%
  filter(Sex == 'female') %>% 
  summarize(meanFare = mean(Fare), Survived = sum(Survived))

# Check out mean Fare & number of survivors grouped by Sex
titanic %>% 
  group_by(Sex) %>% 
  summarise(meanFare = mean(Fare), Survived = sum(Survived))

# Check out mean Fare & proportion of survivors grouped by Sex
titanic %>% 
  group_by(Sex) %>% 
  summarise(meanFare = mean(Fare), Survived = sum(Survived)/n())
